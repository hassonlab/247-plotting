function ntools_elec_plot_onebrain2(varargin)
...
%             disp(colors);
            ...
            effect = elec_all.Var6;
            colors = [zeros(size(effect,1),1) zeros(size(effect,1),1) zeros(size(effect,1),1)];
            
            effect1 = effect(effect < 0);
            effect1(effect1 < -0.5) = -0.5;
            adj_effect1 = effect1 -  min(effect1);
            col_dif1 = 1 / max(adj_effect1);
%             adj_effect1 = effect1 + 1;
%             col_dif1 = 1 / 1;
            col_mark1 = adj_effect1 * col_dif1;

            effect2 = effect(effect > 0);
            effect2(effect2 > 0.5) = 0.5;
            adj_effect2 = effect2 - min(effect2);
            col_dif2 = 1 / max(adj_effect2);
%             col_dif2 = 1 / 1;
            col_mark2 = adj_effect2 * col_dif2;

            colors1 = [col_mark1 col_mark1 ones(size(col_mark1,1),1)];
            colors(effect < 0,:) = colors1;
            colors2 = [ones(size(col_mark2,1),1) 1-col_mark2 1-col_mark2];
            colors(effect > 0,:) = colors2;
...
if exist('cbar_title','var')
    ...
    map1 = [ones(length(map_colors),1) (1 - map_colors)' (1 - map_colors)'];
    map2 = [(map_colors)' (map_colors)' ones(length(map_colors),1) ];
    map = flip([map1;map2]);
    %map = [ones(length(map_colors),1) (1 - map_colors) zeros(length(map_colors),0)];
    colormap(map);
%all_colors(51,:)
%        colormap(all_colors);
    caxis([0 1])
    if exist('tick_labels','var')
        ...
        maxMinDiff = (ceil(max(effect1)*100)/100) - (floor(min(effect1)*100)/100);
%         maxMinDiff = (ceil(max(effect1)*100)/100) - (floor(-1*100)/100);
        stepSize = maxMinDiff/2;
        ticks1 = (floor(min(effect1)*100)/100):stepSize:(ceil(max(effect1)*100)/100);
%         ticks1 = (floor(-1*100)/100):stepSize:(ceil(max(effect1)*100)/100);

        maxMinDiff = (ceil(max(effect2)*100)/100) - (floor(min(effect2)*100)/100);
%         maxMinDiff = (ceil(1*100)/100) - (floor(min(effect2)*100)/100);
        stepSize = maxMinDiff/2;
        ticks2 = (floor(min(effect2)*100)/100):stepSize:(ceil(max(effect2)*100)/100);
%         ticks2 = (floor(min(effect2)*100)/100):stepSize:(ceil(1*100)/100);

        ticks = [ticks1(1:2) ticks2];
