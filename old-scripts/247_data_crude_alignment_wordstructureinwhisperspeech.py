# -*- coding: utf-8 -*-
"""247-DATA-crude-alignment-WORDStructureInWhisperSpeech.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1f3c9G-IbUqmnEaqmEJ76FWan4oepd97l
...
#dirname_plot = "/content/drive/MyDrive/COURSES/8_2023_SPRING/thesis_figs"
...
# get this dict by using the below curl command:
...
projections = projections.assign(phonemes = phonemes, words = words, word_index = word_index, phonemic_index = phonemic_index, 
                                 placeOfArtic = place_of_articulation_save, mannerArtic = manner_articulation_save,
                                 voiced_or_voiceless = voiced_or_voiceless_save, function_or_content = function_or_content_save, 
                                 part_of_speech = part_of_speech_save)

fig = px.scatter(
    projections, x=0, y=1,
    color=projections.part_of_speech, labels={'color': 'part_of_speech'},
    hover_data=["words", "word_index", "phonemes", "phonemic_index", "placeOfArtic", "mannerArtic", "voiced_or_voiceless", "function_or_content", "part_of_speech"],
    color_discrete_sequence=colors_distinct, 
    title = f"whisperSpeech, colored by manner of articulation of first phoneme of word, layer {layer}"
)
fig.write_html(f"{dirname_plot}/whisperSpeech-ColoredByPartOfSpeech_layer0{layer}.html")
...
fig = px.scatter(
    projections, x=0, y=1,
    color=projections.function_or_content, labels={'color': 'function_or_content'},
    hover_data=["words", "word_index", "phonemes", "phonemic_index", "placeOfArtic", "mannerArtic", "voiced_or_voiceless", "function_or_content", "part_of_speech"],
    color_discrete_sequence=colors_distinct, 
    title = f"whisperSpeech, colored by place of articulation of first phoneme of word, layer {layer}"
)
fig.write_html(f"{dirname_plot}/whisperSpeech-ColoredByFunction_Or_Content_layer0{layer}.html")
