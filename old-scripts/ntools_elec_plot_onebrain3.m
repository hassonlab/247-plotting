function ntools_elec_plot_onebrain3(varargin)

fprintf('\n WARNING: do not use right now unless your name is Bobbi')
...
if strcmp(effect,'effect')    
    if exist(fullfile(PathName, FileName),'file')
        if strcmp(FileName(end-3:end),'.csv')
            ...
            elec_all.Properties.VariableNames = {'electrodeName','X','Y','Z','type','effect','region'};
            %elec_all = elec_all(strcmp(elec_all.type,'G') | strcmp(elec_all.type,'EG'),:);
            %elec_all = elec_all(contains(elec_all.region,'STG'),:);
            elec_all_temp = readtable(fullfile(PathName, 'NY625_aphasiaMaxEncoding_prod.csv'));
            elec_all_temp2 = readtable(fullfile(PathName, 'NY625_beforeMaxEncoding_prod.csv'));
            elec_all_temp.Properties.VariableNames = {'electrodeName','X','Y','Z','type','effect','region'};
            elec_all_temp2.Properties.VariableNames = {'electrodeName','X','Y','Z','type','effect','region'};
            elec_all(95,:) = [];
            elec_all_temp(95,:) = [];
            elec_all_temp2(95,:) = [];
            %elec_all_temp = elec_all_temp(contains(elec_all_temp.region,'STG'),:);
            %elec_all_temp2 = elec_all_temp2(contains(elec_all_temp2.region,'STG'),:);
            %elec_all_temp = elec_all_temp(strcmp(elec_all_temp.type,'G') | strcmp(elec_all_temp.type,'EG'),:);
            %elec_all_temp2 = elec_all_temp2(strcmp(elec_all_temp2.type,'G') | strcmp(elec_all_temp2.type,'EG'),:);
            elec_all.effect = elec_all.effect - elec_all_temp2.effect;
            ...
            %fclose(fid);
%             elec_all = elec_all(elec_all.effect >= 0.1,:);
            elec_all = elec_all(elec_all.effect <= -0.1,:);
        end
    end

            effect = elec_all.effect;
%             effect(effect > 0.5) = 0.5;
%             adj_effect = effect -  0;
            effect(effect < -0.5) = -0.5;
            adj_effect = effect + 0.5;
            col_dif = 1 / 0.5; %2.4009;
            col_mark = adj_effect  * col_dif;
%             colors = [ones(size(col_mark,1),1) 1-col_mark 1-col_mark];
            colors = [col_mark col_mark ones(size(col_mark,1),1)];

elseif strcmp(effect,'colors')
    %if strcmp(FileName(end-3:end),'.csv')
        elec_all = readtable(fullfile(PathName, FileName));
        elec_all.Properties.VariableNames = {'electrodeName','X','Y','Z','type','c1','c2','c3'};
        %elec_all = elec_all(elec_all.glove_sigElec == 1,:);
...
%         end
    %end
        colors = [elec_all.c1 elec_all.c2 elec_all.c3];
...
%if strcmp(FileName(end-3:end),'.csv')
%elec_all.Properties.VariableNames = {'electrodeName','X','Y','Z','type'};
...
% a = ' before > after';
% b = ' after > before';
% c = ' after > before; before & after onset';
% d = ' after > before before onset; before > after after onset';
% e = ' before > after before onset; no diff after onset';
% f = ' after > before before onset; no diff before onset';
% % g = ' Participant 3';
% % h = ' Participant 2';
% % i = ' Participant 1';
% plotSpheres(-100,-100,40,2,[0.6350 0.0780 0.1840]);
%     %[xx, yy, zz] = adjust_elec_label(a); % default radius = 2
%     text('Position',[-102,-102,42.5],'String',d,'Color', 'k','VerticalAlignment','top');
% plotSpheres(-100,-100,45,2,[0.4660 0.6740 0.1880]);
%     %[xx, yy, zz] = adjust_elec_label(b); % default radius = 2
%     text('Position',[-102,-102,47.5],'String',c,'Color','k','VerticalAlignment','top');
% plotSpheres(-100,-100,50,2,[0.3010 0.7450 0.9330]);
    %[xx, yy, zz] = adjust_elec_label(b); % default radius = 2
%     text('Position',[-102,-102,52.5],'String',a,'Color','k','VerticalAlignment','top');
% plotSpheres(-100,-100,55,2,[0.6350 0.0780 0.1840]);
    %[xx, yy, zz] = adjust_elec_label(b); % default radius = 2
%     text('Position',[-102,-102,57.5],'String',b,'Color','k','VerticalAlignment','top');
%plotSpheres(-100,-100,60,2,[0.6350 0.0780 0.1840]);
%     %[xx, yy, zz] = adjust_elec_label(b); % default radius = 2
...
if exist('cbar_title','var')
    ...
    %r1 = 1/s1;
    %r2 = 1/s2;
    %map_colors1 = flip(0:r1:1);
    %map_colors2 = flip(0:r2:1);
%     map = [ones(length(map_colors),1) (1 - map_colors') (1 - map_colors')];
    map = [(map_colors)' (map_colors)' ones(length(map_colors),1)];
    %length(map_colors1)
    %length(map_colors2)
    %map1 = [ones(length(map_colors2),1) map_colors2' map_colors2'];
    %map2 = [(1-map_colors1') (1-map_colors1') ones(length(map_colors1),1)];
    %map = [map2;map1];
    %map = sort(colors);
    map = flip(map);
    colormap(map);
...
%        colormap(all_colors);
    ...
    if exist('tick_labels','var')
        ...
        elec_all_temp = elec_all;
        elec_all_temp = elec_all_temp.effect;
        %elec_all_temp = elec_all_temp(elec_all_temp < 0);
        %maxMinDiff = (ceil(max(elec_all_temp)*100)/100) - (floor(min(elec_all_temp)*100)/100);
        maxMinDiff = 0.5;
        stepSize = maxMinDiff/4;
        %ticks = (floor(min(elec_all_temp)*100)/100):stepSize:(ceil(max(elec_all_temp)*100)/100);
%         ticks = (floor(0*100)/100):stepSize:(ceil(0.5*100)/100);
        ticks = (floor(-0.5*100)/100):stepSize:(ceil(0*100)/100);
...
%         elec_all_temp = elec_all_temp.effect;
%         %elec_all_temp = elec_all_temp(elec_all_temp > 0);
%         maxMinDiff = (ceil(max(max(Effects))*100)/100) - (floor(min(min(Effects))*100)/100);
%         stepSize = maxMinDiff/8
%         ticks = (floor(min(min(Effects))*100)/100):stepSize:(ceil(max(max(Effects))*100)/100);
%         ticks_new = num2str(ticks');
        %ticks_new_3 = cellstr(ticks_new);
        %R = 1/length([ticks_new_2' ticks_new_3']);
