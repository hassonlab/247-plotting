...
def save_file(df, filename):
    ...
    for key, val in areas.items():
        ...
        df_area.to_csv(f"data/plotting/ccn-sig-file-{filename}-{key}.csv", index=False)
...
def main():
    ...
    cors_en.loc[cors_en.loc[:, 0:400].max(axis=1) >= np.mean(en_thresh), "sig_mean"] = 1
    cors_en.loc[cors_en.loc[:, 0:400].max(axis=1) >= np.max(en_thresh), "sig_max"] = 1
    ...
    cors_de.loc[cors_de.loc[:, 0:400].max(axis=1) >= de_thresh_mean, "sig_mean"] = 1
    cors_de.loc[cors_de.loc[:, 0:400].max(axis=1) >= de_thresh_max, "sig_max"] = 1
    ...
    gpt2_sig.loc[:, "elec"] = gpt2_sig.subject.astype(str) + "_" + gpt2_sig.electrode
    gpt2_sig.loc[:, "gpt2"] = 1
