    ...
    for format in formats:
        ...
        for row, _ in df.iterrows():
            ...
            if type == "max":
                ...
                df.loc[row, col_name2] = get_max_before(
                    comp_name, formats[format]
                )
            elif type == "max_after":
                df.loc[row, col_name2] = get_max_after(
                    comp_name, formats[format]
                )
            elif type == "max_idx":
                df.loc[row, col_name2] = get_maxidx(
                    comp_name, formats[format], lags
                )
            elif type == "lags":
