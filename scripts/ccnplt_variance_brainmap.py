...
def main():
    ...
    def get_variance_ratio(layer_max):
        ...
        sgn_variance_concat = layer_max.iloc[0, :] ** 2 * np.sign(layer_max.iloc[0, :])
        sgn_variance_01 = layer_max.iloc[1, :] ** 2 * np.sign(layer_max.iloc[1, :])
        sgn_variance_24 = layer_max.iloc[2, :] ** 2 * np.sign(layer_max.iloc[2, :])
        sgn_variance_common = sgn_variance_01 + sgn_variance_24 - sgn_variance_concat
    ...
    df = df.merge(var_ratio.rename("var_ratio"), left_index=True, right_index=True)
    breakpoint()
    df.loc[:, 0] = df.index
